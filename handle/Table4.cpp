...
void dfsOutput(Node *node, string to_output, ofstream &outfile);
double generateDegree(Node *root, ofstream &to_next_table, ofstream &outfile);
...
void table4(vector<pair<string, vector<pair<vector<double>, pair<string, int>>>>> score_match_pair)
{
    ...
    if (!infile)
    {
        cout << "未能成功读取rows_to_table2.dat！" << endl;
        return;
    }
    ...
    ofstream outfile;
    outfile.open("cache/new_table4.dat", ios::out | ios::trunc);
    if (!outfile)
    {
        cout << "dfsOutput出错！" << endl;
        system("pause");
    }
    ...
    for (auto i : kcmbs)
    {
        string to_output;
        to_output.clear();
        dfsOutput(i, to_output, outfile);
        // 这两个遍历结构不同，因此只能分开
        dcd.push_back(generateDegree(i, to_next_table, outfile));
    }
...
double generateDegree(Node *root, ofstream &to_next_table, ofstream &outfile)
{
    ...
    outfile << "课程目标：" << root->name << "的达成度为：" << avgfz / zfz << endl;
    return avgfz / zfz;
}