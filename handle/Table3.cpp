...
void dfs(Node *tm, vector<string> to_output, ofstream &outfile);
...
vector<pair<string, vector<pair<vector<double>, pair<string, int>>>>> table3()
{
    ...
    ifstream infile;
    infile.open("samples/sample_table3.txt", ios::in);
    if (!infile)
    {
        cout << "未能成功读入文件！table3未能读入" << endl;
        system("pause");
    }
    cout << "请输入考核环节数：";
    ...
    {
        // ifstream infile2;
        // infile.open("samples/sample_student.txt", ios::in);
        // if (!infile)
        // {
        //     cout << "未能成功读入文件！table3未能读入" << endl;
        // }
...
{
    ofstream outfile2;
    outfile2.open("cache/new_table3.dat", ios::out | ios::trunc);
    if (!outfile2)
    {
        cout << "未能成功写入文件！table3未能成功写入" << endl;
        return;
    }
    for (int i = 0; i < khhj.size(); i++)
    {
        ...
        outfile2 << khhj[i] << '-' << cjzb[i] << '-' << "总分" << '-' << tm[i].second[0];
        outfile2 << "\t学生分数（按序号排列）：" << endl;
        if (tm[i].first->otd)
            dfs(tm[i].first, to_output, outfile2);
        for (int j = 1; j < tm[i].second.size(); j++)
        {
            if (j == 1)
                outfile2 << tm[i].second[j];
            else
                outfile2 << '-' << tm[i].second[j];
        }
        outfile2 << endl;
 ...
 */
void dfs(Node *node, vector<string> to_output, ofstream &outfile)
{
    if (node->ind != 0)
    {
    ...
    if (node->otd == 0)
    {
        ...
        outfile << '-' << node->scores[0]; // 最后输出叶子结点的分数
        outfile << "\t学生分数（按序号排列）：";
        for (int j = 1; j < node->scores.size(); j++)
        {
            if (j == 1)
                outfile << node->scores[j];
            else
                outfile << '-' << node->scores[j];
        }
        outfile << endl;
